# utils_ffmpeg.py (safe DB access version)
import os
import cv2
import time
import numpy as np
...
def process_camera_stream_ffmpeg(camera, schedules, face_analyzer, embeddings_map):
    ...
    try:
        ...
        logger.info(f"\U0001f9d1‍\U0001f393 Active students: {active_students}")
        ...
        for face in faces:
            ...
            if best_score >= MATCH_THRESHOLD and best_h_code:
                try:
                    ...
                    if LOG_MATCHES:
                        logger.info(f"\U0001f9ec Best match {display_name} | Score: {best_score:.4f}")
