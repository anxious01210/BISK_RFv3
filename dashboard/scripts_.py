# dashboard/scripts.py (The dashboard det_set value acceptance fix)
import subprocess
import os
import signal
import time
...
def run_script_by_type(script_type, det_set):
    ...
    if script_path:
        env = os.environ.copy()
        env["DET_SET"] = det_set  # ✅ Pass det_set as an environment variable

        args = ['python3', script_path]
        SCRIPT_PROCESS = subprocess.Popen(args, env=env, preexec_fn=os.setsid)
